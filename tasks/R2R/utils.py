...
def dump_transformer_index(encoder_type, bert_type, splits):
    if encoder_type == 'bert':
        dump_bert_index(splits, bert_type)
...
def dump_bert_index(splits, bert_type='small'):
    from pytorch_pretrained_bert import BertTokenizer
    from nltk.tokenize import sent_tokenize

    # no difference between bert-large and bert-base tokenizers
    if bert_type == 'small':
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    elif bert_type == 'large':
        tokenizer = BertTokenizer.from_pretrained('bert-large-uncased')
    elif bert_type == 'bert_large.bin':
        tokenizer = BertTokenizer.from_pretrained('bert-large-uncased')
    #splits = ['train', 'val_seen', 'val_unseen', 'test']
    ...
    for split in splits:
        ...
        if bert_type == 'small' or bert_type == 'bert_large.bin':
            write_vocab(indexed_tokens, 'tasks/R2R/data/R2R_%s_bert.txt' % split)
        elif bert_type == 'large':
            write_vocab(indexed_tokens, 'tasks/R2R/data/R2R_%s_bert_large.txt' % split)


def load_datasets(splits, encoder_type, bert_type='small'):
    data = []
    for split in splits:
        with open('tasks/R2R/data/R2R_%s.json' % split) as f:
            data += json.load(f)

        if encoder_type in ['bert', 'gpt']:
            filename = 'tasks/R2R/data/R2R_%s_%s.txt' % (split, encoder_type)

            if encoder_type == 'bert' and bert_type == 'large':
                filename = 'tasks/R2R/data/R2R_%s_%s_%s.txt' % (split, encoder_type, bert_type)

            if not os.path.exists(filename):
                dump_transformer_index(encoder_type, bert_type, [split])
