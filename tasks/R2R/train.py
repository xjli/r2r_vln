...
from model import EncoderLSTM, EncoderMultiLSTM, BertEncoder, MultiBertEncoder, GptEncoder, MultiGptEncoder,\
    TransformerEncoder, MultiTransformerEncoder, AttnDecoderLSTM
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hidden_size', dest='hidden_size', default=512, type=int, help='decoder hidden_size')
    parser.add_argument('--enc_hidden_size', dest='enc_hidden_size', default=512, type=int, help='encoder hidden_size')
    ...
    parser.add_argument('--n_iters', dest='n_iters', default=50000, type=int, help='n_iters')
    ...
    parser.add_argument('--ss_n_iters', dest='ss_n_iters', default=-1, type=int, help='scheduled sampling n_iters')
    ...
    parser.add_argument('--log_every', dest='log_every', default=100, type=int, help='log_every')
    parser.add_argument('--save_ckpt', dest='save_ckpt', default=-1, type=int, help='dump model checkpoint, default -1')
    parser.add_argument('--acc_gradient_steps', dest='acc_gradient_steps', default=1, type=int, help='the steps of accumulated gradient')

    parser.add_argument('--schedule_ratio', dest='schedule_ratio', default=0.5, type=float, help='ratio for sample or teacher')
    parser.add_argument('--schedule_anneal', dest='schedule_anneal', action='store_true', help='schedule_ratio is annealling or not')
    parser.add_argument('--dropout_ratio', dest='dropout_ratio', default=0.5, type=float, help='dropout_ratio')
    parser.add_argument('--temp_alpha', dest='temp_alpha', default=1.0, type=float, help='temperate alpha for softmax')
    parser.add_argument('--learning_rate', dest='learning_rate', default=0.0001, type=float, help='learning_rate')
    parser.add_argument('--sc_learning_rate', dest='sc_learning_rate', default=0.00002, type=float, help='sc_learning_rate')
    parser.add_argument('--weight_decay', dest='weight_decay', default=0.0005, type=float, help='weight_decay')
    parser.add_argument('--optm', dest='optm', default='Adam', type=str, help='Adam, Adamax, RMSprop')
    ...
    parser.add_argument('--feedback_method', dest='feedback_method', type=str, default='sample', help='sample or teacher')
    parser.add_argument('--bidirectional', dest='bidirectional', type=boolean_string, default=False, help='bidirectional')
    ...
    parser.add_argument('--action_space', dest='action_space', type=int, default=6, help='6 or -1(navigable viewpoints)')
    ...
    parser.add_argument('--att_ctx_merge', dest='att_ctx_merge', type=str, default='None', help='mean cat mean sum (to merge attention)')
    ...
    parser.add_argument('--clip_gradient_norm', dest='clip_gradient_norm', type=float, default=40, help='clip gradient norm')
    ...
    parser.add_argument('--dec_h_type', dest='dec_h_type', type=str, default='none', help='none or vc')

    parser.add_argument('--encoder_type', dest='encoder_type', type=str, default='lstm', help='lstm transformer bert or gpt')
    parser.add_argument('--top_lstm', dest='top_lstm', type=boolean_string, default=False, help='add lstm to the top of transformers')
    parser.add_argument('--transformer_update', dest='transformer_update', type=boolean_string, default=True, help='update Bert')
...
use_bert = (encoder_type in ['bert', 'gpt'])  # for tokenizer and dataloader
...
agent_params['acc_gradient_steps'] = params['acc_gradient_steps']
...
def train(train_env, train_Eval, encoder, decoder, monotonic, n_iters, resume_split_arr, cur_split_arr, score_name, jump_iters, log_every=log_every, val_envs={}, n_iters_resume=0):
    ''' Train on training set, validating on both seen and unseen. '''

    data_log = defaultdict(list)

    agent = Seq2SeqAgent(train_env, "", encoder, decoder, 'resume' if n_iters_resume>0 else 10, aux_ratio, decoder_init, agent_params, monotonic, max_episode_len)
    ...
    while idx+log_every < n_iters:
        ...
        print("EVALERR: {}%".format(100 * best_spl))
...
def test_submission():
    ...
    train_env = R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=batch_size,
                         splits= submit_splits, tokenizer=tok, att_ctx_merge=att_ctx_merge, bert_type=bert_type)
    ...
    test_env = R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=batch_size, splits=['test'],
                        tokenizer=tok, att_ctx_merge=att_ctx_merge, bert_type=bert_type)
...
def train_val(n_iters_resume=0):
    ...
    val_envs = {split: (R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=batch_size,
             splits=[split], tokenizer=tok, att_ctx_merge=att_ctx_merge, bert_type=bert_type), Evaluation([split], encoder_type)) for split in val_splits}

    # Create training env
    train_env = R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=batch_size,
                          splits=train_splits, tokenizer=tok, att_ctx_merge=att_ctx_merge, bert_type=bert_type)
    ...
    if use_pretrain:
        ...
        pretrain_env = R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, batch_size=batch_size,
                          splits=pretrain_splits, tokenizer=tok, att_ctx_merge=att_ctx_merge, min_n_sentences=pretrain_n_sentences, bert_type=bert_type)
        ...
        if (agent_params['schedule_ratio'] > 0 and agent_params['schedule_ratio'] < 1.0) and ss_n_pretrain_iters > 0:
            ...
            best_model_ss_pretrain_iter = train(pretrain_env, None, encoder, decoder, None, ss_n_pretrain_iters, resume_splits_array, cur_splits_array,
                                                pretrain_score_name, last_train_iters, log_every, val_envs=val_envs, n_iters_resume=n_iters_pretrain_resume)
        ...
        if train_in_pretrain:
            ...
            n_iters_resume = train(pretrain_env, None, encoder, decoder, None, pretrain_n_iters, resume_splits_array, cur_splits_array, #pretrain_env.splits,
                               pretrain_score_name, last_train_iters, val_envs=val_envs, n_iters_resume=n_iters_pretrain_resume)
    ...
    if 0 < agent_params['schedule_ratio'] < 1.0 and ss_n_iters > 0:
        ...
        best_model_ss_iter = train(train_env, train_Eval, encoder, decoder, monotonic, ss_n_iters, resume_splits_array, cur_splits_array, #resume_splits,
                                   train_score_name, last_train_iters if use_pretrain else -1, val_envs=val_envs, n_iters_resume=n_iters_resume)
    ...
    best_model_iter = train(train_env, train_Eval, encoder, decoder, monotonic, n_iters, resume_splits_array, cur_splits_array, #resume_splits,
                            train_score_name, last_train_iters if use_pretrain else -1, val_envs=val_envs, n_iters_resume=n_iters_resume)
    return best_model_iter
...
def load_test(n_iters_resume):
    ...
    val_envs = {split: (R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, beam_size, batch_size=batch_size, splits=[split],
                                 tokenizer=tok, att_ctx_merge=att_ctx_merge, bert_type=bert_type), Evaluation([split], encoder_type)) for split in val_splits}
    ...
    if hasattr(encoder, 'n_sentences') and single_sentence_test:
        ...
        R2RBatch(feature_store, nav_graphs, panoramic, action_space, ctrl_feature, encoder_type, beam_size=beam_size, batch_size=batch_size,
                 splits=[split], tokenizer=tok, att_ctx_merge='Eval', bert_type=bert_type), Evaluation([split], encoder_type)) for split in val_splits} #if split!='test'}
        loss_str, data_log = '', defaultdict(list)
